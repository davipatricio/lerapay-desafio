...
export class AddDomainEntities1786874696359 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`checkout_links\` (\`id\` varchar(36) NOT NULL, \`user_id\` varchar(36) NOT NULL, \`external_reference\` varchar(255) NOT NULL, \`title\` varchar(255) NOT NULL, \`amount\` int NOT NULL COMMENT 'Valor em centavos', \`allowed_methods\` text NOT NULL, \`max_installments\` int NOT NULL DEFAULT '12', \`status\` enum ('ACTIVE', 'EXPIRED', 'COMPLETED') NOT NULL DEFAULT 'ACTIVE', \`expires_at\` datetime NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_bb1de397a5896527ac6336b6cd\` (\`user_id\`), UNIQUE INDEX \`IDX_9964632142499d5ca4fde5c292\` (\`external_reference\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`orders\` (\`id\` varchar(36) NOT NULL, \`checkout_link_id\` varchar(36) NULL, \`user_id\` varchar(36) NOT NULL, \`external_reference\` varchar(255) NOT NULL, \`amount\` int NOT NULL COMMENT 'Valor em centavos', \`method\` enum ('PIX', 'CREDIT_CARD') NOT NULL, \`status\` enum ('PENDING', 'APPROVED', 'DENIED', 'CANCELLED', 'EXPIRED') NOT NULL DEFAULT 'PENDING', \`gateway_payment_id\` varchar(255) NULL, \`installments\` int NOT NULL DEFAULT '1', \`fee_percent\` decimal(5,2) NOT NULL DEFAULT '0.00', \`fee_amount\` int NOT NULL COMMENT 'Valor da taxa em centavos' DEFAULT '0', \`net_amount\` int NOT NULL COMMENT 'Valor líquido em centavos' DEFAULT '0', \`qr_code\` text NULL, \`qr_code_base64\` longtext NULL, \`payer_name\` varchar(255) NULL, \`payer_document\` varchar(20) NULL, \`expires_at\` datetime NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_d0603b3a3b68935fdfbfc45af7\` (\`checkout_link_id\`), INDEX \`IDX_a922b820eeef29ac1c6800e826\` (\`user_id\`), INDEX \`IDX_329868121e92be672262efea85\` (\`external_reference\`), INDEX \`IDX_20240295c7bdb45e9203747c99\` (\`gateway_payment_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`transactions\` (\`id\` varchar(36) NOT NULL, \`user_id\` varchar(36) NOT NULL, \`order_id\` varchar(36) NULL, \`gateway_transaction_id\` varchar(255) NULL, \`external_reference\` varchar(255) NULL, \`type\` enum ('PIX', 'CREDIT_CARD', 'WITHDRAWAL') NOT NULL, \`status\` enum ('PENDING', 'APPROVED', 'DENIED', 'CANCELLED', 'EXPIRED') NOT NULL DEFAULT 'PENDING', \`amount\` int NOT NULL COMMENT 'Valor em centavos', \`fee\` int NOT NULL COMMENT 'Valor da taxa em centavos' DEFAULT '0', \`net_amount\` int NOT NULL COMMENT 'Valor líquido em centavos' DEFAULT '0', \`description\` varchar(255) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_e9acc6efa76de013e8c1553ed2\` (\`user_id\`), INDEX \`IDX_3cb0558ed36997f1d9ecc1118e\` (\`order_id\`), INDEX \`IDX_16a92060f6215782287f05453f\` (\`gateway_transaction_id\`), INDEX \`IDX_4ef0fc5306621274956d163a17\` (\`external_reference\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
